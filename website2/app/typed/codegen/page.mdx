...
- **`@InstantLink()`** — marks a relation field; see [Relations](../relations/).
...
`@InstantLink` (see [Relations](../relations/)); non-nullable relations are rejected by
...
The generated table behaves like any [typed table](../query-dsl/), and `getAll` /
...
`table.tx(db)` sugar, see [Transactions](../transactions/).
